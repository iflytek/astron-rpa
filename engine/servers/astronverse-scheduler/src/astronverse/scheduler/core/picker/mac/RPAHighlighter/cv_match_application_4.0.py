...
def debug_log(message):
    try:
        ...
        log_path = os.path.join(log_dir, "highlighter-debug.log")
...
class HighlightForm(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowFlags(self._overlay_window_flags())
    ...
    @staticmethod
    def _overlay_window_flags():
        if sys.platform == "darwin":
            flags = Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint | Qt.Tool | Qt.WindowDoesNotAcceptFocus
        else:
            flags = (
                Qt.WindowTransparentForInput | Qt.WindowStaysOnTopHint | Qt.FramelessWindowHint | Qt.Dialog | Qt.Tool
            )
    ...
    def paintEvent(self, event):
        ...
        debug_log(
            f"paintEvent mode={self.mode} draw_rect={self.draw_rect.getRect()} "
            f"widget_geom={self.geometry().getRect()} visible={self.isVisible()}"
        )
        print("paintEvent", self.mode, self.draw_rect)
        if self.mode != "validate":
            pen = QPen(QColor(255, 255, 224))
            pen.setWidth(5 if sys.platform == "darwin" else 3)
            if sys.platform == "darwin":
    ...
    def update_rect(self, rect):
        if sys.platform == "darwin":
            ...
            self.setGeometry(rect.left() - padding, rect.top() - padding, width + padding * 2, height + padding * 2)
            self.draw_rect = QRect(padding, padding, width, height)
        else:
            self.draw_rect = rect

        debug_log(
            f"update_rect input={rect.getRect()} draw_rect={self.draw_rect.getRect()} "
            f"widget_geom={self.geometry().getRect()} visible={self.isVisible()}"
        )
    ...
    def update_mode(self, mode):
        ...
        if self.mode.find("CV") != -1:
            ...
            if sys.platform == "darwin":
                ...
                flags = Qt.WindowStaysOnTopHint | Qt.FramelessWindowHint | Qt.Dialog | Qt.Tool
            if sys.platform.startswith("linux"):
                flags |= Qt.X11BypassWindowManagerHint
...
class ConsoleApp(QMainWindow):
    def __init__(self, socket_port):
        super().__init__()
        debug_log(f"ConsoleApp init socket_port={socket_port} platform={sys.platform}")
        ...
        debug_log(
            f"highlight_form created visible={self.highlight_form.isVisible()} "
            f"geom={self.highlight_form.geometry().getRect()}"
        )
        self.ctrl_form = None
        self.hint_form = None
    ...
    def read_datagrams(self):
        ...
        if self.udp_socket.hasPendingDatagrams():
            ...
            if datagram:
                ...
                debug_log(f"received sender={sender_host}:{sender_port} message={message}")
