...
class AXUIFactory:
    ...
    def find(cls, ele: dict, picker_type: str, **kwargs) -> Union[list[AXUILocator], AXUILocator, None]:
        ...
        for pid in pids:
            try:
                ...
                while node_idx < len(path_list):
                    node_dict = path_list[node_idx]
                    ...
                    logger.debug(
                        f"第 {current_level} 层匹配条件 → tag={match_params['tag_name']}, cls={match_params['cls_name']}, "
                        f"name={match_params['name']}, value={match_params['value']}, index={match_params['expected_idx']}"
                    )

                    next_list = cls._collect_matches(search_list, match_params)

                    if not next_list:
                        next_checked = cls._next_checked_node(path_list, node_idx + 1)
    ...
    @classmethod
    def _next_checked_node(cls, path_list: list, start_idx: int):
        for idx in range(start_idx, len(path_list)):
            node = path_list[idx]
