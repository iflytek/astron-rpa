...
class ExcelCore(IExcelCore):
    ...
    def get_exist_excel(cls, file_name):
        ...
        """
        excel_obj_list = []
        excel_flag, wps_flag = cls.check_excel_exists()
        ...
        if len(excel_obj_list) == 1:
            ...
            raise Exception(f"检测到对象：{file_name}在WPS/Office中打开,需关闭其中一个")
        else:
            raise Exception(f"不存在已打开的Excel文件:{file_name}")
    ...
    def cell_type(
        ...
        if design_type == ReadRangeType.CELL:
            ...
            used_row = ws.Cells.SpecialCells(11).Row
            if type(col) is int:
                cell_position = f"{col}1:{col}{used_row}"
                cell_positions.append(cell_position)
            else:
                col = col.replace("，", ",")
                if col.find(":") != -1:
                    ...
                    cell_positions.append(f"{col[0]}1:{col[1]}{used_row}")
                elif col.find(",") != -1:
                    col = col.split(",")
                    for index in range(len(col)):
                        col[index] = cls._handle_column_input(col[index], used_col)
                        cell_positions.append(f"{col[index]}1:{col[index]}{used_row}")
                else:
            ...
            cell_position = f"{data_range_handle[0]}{data_range_handle[1]}:{data_range_handle[2]}{data_range_handle[3]}"
            cell_positions.append(cell_position)
    ...
    def clear_content(
        ...
        try:
            ...
            if "无法对合并单元格执行此操作" in str(err):
                raise ValueError("当前单元格区域存在合并单元格情况，无法清空内容，重新输入最大合并区域") from err
            else:
                raise err
            ...
            if insert_type in [
                ...
                if row_direction == RowDirectionType.UPPER:  # 向上
                    for _ in range(insert_num):
                        range_obj = ws_obj.Range(f"A{str(row)}:{cls._column_number_to_letter(used_col)}{str(row)}")
                        range_obj.EntireRow.Insert(Shift=-4162)
                if row_direction == RowDirectionType.LOWER:  # 向下
                    for _ in range(insert_num):
                        range_obj = ws_obj.Range(
                            f"A{str(row + 1)}:{cls._column_number_to_letter(used_col)}{str(row + 1)}"
                        )
                ...
                for _ in range(insert_num):
                    if col_direction == ColumnDirectionType.LEFT:  # 左
                        ...
                        range_obj = ws_obj.Range(f"{col_letter}1:{col_letter}{used_row}")
                    ...
                    if col_direction == ColumnDirectionType.RIGHT:  # 右
                        ...
                        range_obj = ws_obj.Range(f"{new_col}1:{new_col}{used_row}")
                        range_obj.EntireColumn.Insert(Shift=-4159)
                row = 1
                col = new_col
                if not blank_rows:
    ...
    def _search_sheet(
        ...
        if search_range == SearchRangeType.ALL:
            ...
            for extract_row in rows:
                cell_positions.append(f"A{extract_row}:{cls._column_number_to_letter(used_col)}{extract_row}")
            ...
            for extract_col in cols:
                cell_positions.append(
                    f"{cls._column_number_to_letter(extract_col)}1:{cls._column_number_to_letter(extract_col)}{used_row}"
                )
            ...
            cell_positions.append(ws.Range(f"{start_col}{start_row}:{end_col}{end_row}"))
    ...
    def loop_content(
        ...
        if select_type in [SearchRangeType.ROW, SearchRangeType.AREA]:
            start_row = cls._handle_row_input(start_row, used_row)
            end_row = cls._handle_row_input(end_row, used_row)
    ...
    def insert_formula(
        ...
        if insert_direction == InsertFormulaDirectionType.RIGHT:
            ...
            end_col = cls._handle_column_input(end_col, used_col, False)
            row = cls._handle_row_input(row, used_row)
            starter = ws_obj.Range(f"{start_col}{str(row)}")
            starter.Value = formula
            starter.AutoFill(
                ws_obj.Range(f"{start_col}{str(row)}:{end_col}{str(row)}"),
                0,
            )
    ...
    def text_to_number(
        ...
        empty_cell = f"{cls._column_number_to_letter(used_col)}{used_row + 1}"
        for cell_position in cell_positions:
            for item in ws.Range(cell_position).Cells:
                ...
                ws.Range(empty_cell).Value = f"=VALUE({item_address})"
    ...
    def set_row_height(cls, excel_obj, sheet_name: str, set_type: SetType, row: str, height: float):
        ...
        if set_type == SetType.VALUE:  # 指定行高
            for irow in row_list:
                ws.Rows(irow).RowHeight = height
        elif set_type == SetType.AUTO:  # 自动调整
            for irow in row_list:
                ws.Rows(irow).AutoFit()
    ...
    def get_or_create_sheet_and_pivot_table(excel_obj, pivot_sheet, pivot_start_cell, pivot_table_name, src_range):
        try:
            ...
            logger.info(f"get_or_create_sheet_and_pivot_table {e}")
