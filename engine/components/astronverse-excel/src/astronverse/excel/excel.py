...
class Excel:
    ...
    @atomicMg.atomic(
        "Excel",
        inputList=[
            atomicMg.param("file_path", formType=AtomicFormTypeMeta(type=AtomicFormType.INPUT_VARIABLE_PYTHON_FILE.value,
                                                                    params={"filters": [".xlsx", ".xls"],
                                                                            "file_type": "file"})),
    ...
    def open_excel(file_path: PATH = "",
                   ...
                   password: str = "",
                   update_links: bool = True) -> ExcelObj:
        if not os.path.exists(file_path):
            raise Exception('填写的文件路径有误，请输入正确的路径！')
        else:
            file_path = os.path.abspath(file_path)
        application = Application.init_app(default_application=default_application, visible_flag=visible_flag, retry=2, retry_delay=1, prefer_existing=False)
        excel = Application.open_workbook(application=application, file_path=file_path, password=password, update_links=update_links)
        return excel
    ...
    def get_excel(file_name) -> ExcelObj:
        ...
        for key in keys:
            try:
                application = Application.init_app(default_application=key, visible_flag=None, retry=0, retry_delay=0, prefer_existing=True)
    ...
    def create_excel(file_path: str = "", file_name: str = "",
                     default_application: ApplicationType = ApplicationType.EXCEL, visible_flag: bool = True,
                     exist_handle_type: FileExistenceType = FileExistenceType.RENAME, password: str = "") -> Tuple[ExcelObj, str]:
        if not os.path.exists(file_path):
            raise Exception('填写的文件路径有误，请输入正确的路径！')
        else:
            file_path = os.path.abspath(file_path)
        ...
        application = Application.init_app(default_application=default_application, visible_flag=visible_flag, retry=2, retry_delay=1, prefer_existing=True)
    ...
    @atomicMg.atomic(
        "Excel",
        inputList=[
            atomicMg.param("file_path", dynamics=[DynamicsItem(
                key="$this.file_path.show",
                expression="return $this.save_type.value == '{}'".format(SaveType.SAVE_AS.value))
            ], required=False,
                           formType=AtomicFormTypeMeta(type=AtomicFormType.INPUT_VARIABLE_PYTHON_FILE.value,
                                                       params={"filters": [], "file_type": "folder"})),
            atomicMg.param("file_name", dynamics=[DynamicsItem(
                key="$this.file_name.show",
                expression="return $this.save_type.value == '{}'".format(SaveType.SAVE_AS.value))
            ], required=False),
            atomicMg.param("exist_handle_type", dynamics=[DynamicsItem(
                key="$this.exist_handle_type.show",
                expression="return $this.save_type.value == '{}'".format(SaveType.SAVE_AS.value))
            ])
        ],
        outputList=[])
    def save_excel(excel: ExcelObj, save_type: SaveType = SaveType.SAVE, file_path: str = "", file_name: str = "",
                   exist_handle_type: FileExistenceType = FileExistenceType.RENAME, close_flag: bool = False):
    ...
    @atomicMg.atomic(
        ...
        outputList=[])
    def close_excel(close_range_flag: CloseRangeType = CloseRangeType.ONE,
                    excel: ExcelObj = None,
        ...
        if close_range_flag == CloseRangeType.ALL:
            ...
            Excel.save_excel(excel=excel, save_type=save_type_one, file_path=file_path, file_name=file_name, exist_handle_type=exist_handle_type, close_flag=True)
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            ...
            atomicMg.param("edit_type", dynamics=[DynamicsItem(
                key="$this.edit_type.show",
                expression="return $this.edit_range.value == '{}' || $this.edit_range.value == '{}'".format(ReadRangeType.ROW.value, ReadRangeType.COLUMN.value))
            ], required=False),
        ],
        outputList=[])
    def edit_excel(excel: ExcelObj, edit_range: EditRangeType = EditRangeType.ROW, sheet_name: str = "",
                   start_col: str = "A", start_row: str = "1", value: str = "", edit_type: EditType = EditType.OVERWRITE) -> ExcelObj:
    ...
    def read_excel(excel: ExcelObj, sheet_name: str = "", read_range: ReadRangeType = ReadRangeType.CELL,
                   start_col: str = "", end_col: str = "",
                   cell: str = "", row: int = 1, column: str = "", start_row: int = 1, end_row: int = 1,
                   read_display: bool = True, trim_spaces: bool = False, replace_none: bool = True):
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            ...
            atomicMg.param("numberformat_other", dynamics=[DynamicsItem(
                key="$this.numberformat_other.show",
                expression="return $this.numberformat.value == '{}'".format(NumberFormatType.CUSTOM.value))
            ], required=False),
            atomicMg.param("auto_row_height", dynamics=[DynamicsItem(
                key="$this.auto_row_height.show",
                expression="return $this.design_type.value == '{}'".format(ReadRangeType.ROW.value))
            ]),
            atomicMg.param("auto_column_width", dynamics=[DynamicsItem(
                key="$this.auto_column_width.show",
                expression="return $this.design_type.value == '{}'".format(ReadRangeType.COLUMN.value))
            ]),
        ],
        outputList=[])
    def design_cell_type(excel: ExcelObj,
                         sheet_name: str = "",
    ...
    def copy_excel(excel: ExcelObj, sheet_name: str = '', copy_range_type: ReadRangeType = ReadRangeType.CELL,
                   cell_position: str = 'A1',
                   row: str = '', col: str = '', range_position: str = 'A1:B5'):
    ...
    def paste_excel(excel: ExcelObj, sheet_name: str = "", paste_type: PasteType = PasteType.ALL,
                    start_location: str = "A1", skip_blanks: bool = False, transpose: bool = False):
        worksheet = Worksheet.get_worksheet(excel, sheet_name, default=1)
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            ...
            atomicMg.param("direction", dynamics=[DynamicsItem(
        ...
        outputList=[])
    def delete_excel_cell(excel: ExcelObj,
                          sheet_name: str = "",
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            atomicMg.param("sheet_name", required=False),
            atomicMg.param("cell_location", dynamics=[DynamicsItem(
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            ...
            atomicMg.param("row_direction", dynamics=[DynamicsItem(
            ...
            atomicMg.param("col_direction", dynamics=[DynamicsItem(
                key="$this.col_direction.show",
                expression="return $this.insert_type.value == '{}'".format(EnhancedInsertType.COLUMN.value))
            ]),
            atomicMg.param("insert_num", dynamics=[DynamicsItem(
                key="$this.insert_num.show",
                expression="return $this.blank_rows.value == true")
            ]),
            atomicMg.param("insert_content", dynamics=[DynamicsItem(
                key="$this.insert_content.show",
                expression="return $this.blank_rows.value == false")
            ]),
        ],
        outputList=[])
    def insert_excel_row_or_column(excel: ExcelObj,
                                   sheet_name: str = "",
        ...
        if insert_type in [EnhancedInsertType.ROW, EnhancedInsertType.ADD_ROWS]:
            ...
            if row_direction == RowDirectionType.UPPER:  # 向上
                for _ in range(insert_num):
                    r_obj = Worksheet.get_range(worksheet, "A{}:{}{}".format(start_row_num, r_end_col_letter, start_row_num))
                    Range.insert_range(r_obj, "row")
            elif row_direction == RowDirectionType.LOWER:  # 向下
                for _ in range(insert_num):
                    r_obj = Worksheet.get_range(worksheet, "A{}:{}{}".format(start_row_num + 1, r_end_col_letter, start_row_num + 1))
                    Range.insert_range(r_obj, "row")
            ...
            if col_direction == ColumnDirectionType.LEFT:  # 向上
                for _ in range(insert_num):
                    r_obj = Worksheet.get_range(worksheet, "{}1:{}{}".format(column_number_to_letter(start_col_num), column_number_to_letter(start_col_num), r_end_row))
                    Range.insert_range(r_obj, "column")
            elif col_direction == ColumnDirectionType.RIGHT:  # 向下
                for _ in range(insert_num):
                    r_obj = Worksheet.get_range(worksheet, "{}1:{}{}".format(column_number_to_letter(start_col_num + 1), column_number_to_letter(start_col_num + 1), r_end_row))
                    Range.insert_range(r_obj, "column")
    ...
    def get_excel_row_num(excel: ExcelObj, sheet_name: str = '', get_col_type: ColumnType = ColumnType.ALL, col: str = ''):
        worksheet = Worksheet.get_worksheet(excel, sheet_name, default=1)
        used_range = Worksheet.get_worksheet_used_range(worksheet)
        _, _, r_end_row, r_end_col, r_address = used_range
    ...
    def get_excel_col_num(excel: ExcelObj, sheet_name: str = '',
                          get_row_type: RowType = RowType.ALL, row: str = "",
    ...
    def get_excel_first_available_col(excel: ExcelObj, sheet_name: str = '', output_type: ColumnOutputType = ColumnOutputType.LETTER):
        """
        获取第一个空列的列号或字母
    ...
    def loop_excel_content(excel: ExcelObj,
                           sheet_name: str = "",
        ...
        if select_type == SearchRangeType.ROW:
            ...
            data_region = "{}{}:{}{}".format(column_number_to_letter(start_col_num), "1", column_number_to_letter(end_col_num), end_row_num)
        elif select_type == SearchRangeType.AREA:
            data_region = "{}{}:{}{}".format(column_number_to_letter(start_col_num), start_row_num, column_number_to_letter(end_col_num), end_row_num)
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            atomicMg.param("sheet_name", required=False),
            atomicMg.param("relative_sheet_name", dynamics=[DynamicsItem(
        ...
        inputList=[
            atomicMg.param(
                ...
                dynamics=[DynamicsItem(
                    ...
                    expression="return ['{}', '{}'].includes($this.move_type.value)".format(MoveSheetType.MOVE_AFTER.value, MoveSheetType.MOVE_BEFORE.value))
                ]
            )
        ],
        outputList=[])
    ...
    @atomicMg.atomic(
        ...
        inputList=[],
        outputList=[])
    def delete_excel_worksheet(excel: ExcelObj,
                               del_sheet_name: str):
        """
        删除工作表
    ...
    @atomicMg.atomic(
        ...
        inputList=[],
        outputList=[])
    def rename_excel_worksheet(excel: ExcelObj,
    ...
    @atomicMg.atomic(
        ...
        outputList=[])
    def copy_excel_worksheet(excel: ExcelObj,
                             source_sheet_name: str,
                             new_sheet_name: str,
        ...
        # 复制，并设置active
        time.sleep(0.5)
        Worksheet.copy_worksheet(source_worksheet, trigger_excel, location, is_same_workbook=True if CopySheetType.CURRENT_WORKBOOK else False)
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            ...
            atomicMg.param("replace_str", dynamics=[DynamicsItem(
        ...
        outputList=[
            atomicMg.param("search_excel_result", types="Dict")
        ])
    def search_and_replace_excel_content(excel: ExcelObj,
                                         find_str: str,
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            atomicMg.param("sheet_name", required=False),
            atomicMg.param("pic_height", dynamics=[DynamicsItem(
                key="$this.pic_height.show",
                expression="return $this.pic_size_type.value == '{}'".format(ImageSizeType.NUMBER.value))
            ]),
            atomicMg.param("pic_width", dynamics=[DynamicsItem(
                key="$this.pic_width.show",
                expression="return $this.pic_size_type.value == '{}'".format(ImageSizeType.NUMBER.value))
            ]),
            atomicMg.param("pic_scale", dynamics=[DynamicsItem(
                key="$this.pic_scale.show",
                expression="return $this.pic_size_type.value == '{}'".format(ImageSizeType.SCALE.value))
            ]),
            atomicMg.param("pic_path", formType=AtomicFormTypeMeta(type=AtomicFormType.INPUT_VARIABLE_PYTHON_FILE.value,
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            ...
            atomicMg.param("col", dynamics=[DynamicsItem(
                key="$this.col.show",
                expression="return $this.insert_direction.value == '{}'".format(InsertFormulaDirectionType.DOWN.value))
            ]),
            atomicMg.param("start_row", dynamics=[DynamicsItem(
                key="$this.start_row.show",
                expression="return $this.insert_direction.value == '{}'".format(InsertFormulaDirectionType.DOWN.value))
            ...
            atomicMg.param("row", dynamics=[DynamicsItem(
                key="$this.row.show",
                expression="return $this.insert_direction.value == '{}'".format(InsertFormulaDirectionType.RIGHT.value))
            ]),
            atomicMg.param("start_col", dynamics=[DynamicsItem(
                key="$this.start_col.show",
                expression="return $this.insert_direction.value == '{}'".format(InsertFormulaDirectionType.RIGHT.value))
        ...
        outputList=[])
    def insert_formula(excel: ExcelObj,
                       sheet_name: str = '',
        ...
        if insert_direction == InsertFormulaDirectionType.DOWN:
            ...
            target_range = Worksheet.get_range(worksheet, "{}{}:{}{}".format(col_letter, str(start_row_num), col_letter, str(end_row_num)))
        ...
        if insert_direction == InsertFormulaDirectionType.RIGHT:
            ...
            Range.set_range_data(starter, formula)
            target_range = Worksheet.get_range(worksheet, "{}{}:{}{}".format(start_col_letter, str(row_num), end_col_letter, str(row_num)))
            Range.autofill_range(starter, target_range)
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            ...
            atomicMg.param("find_str", dynamics=[DynamicsItem(
                key="$this.find_str.show",
                expression="return $this.comment_type.value == '{}'".format(CreateCommentType.CONTENT.value))
            ]),
            atomicMg.param("sheet_name", required=False, dynamics=[DynamicsItem(
                key="$this.sheet_name.show",
            ...
            atomicMg.param("comment_range", dynamics=[DynamicsItem(
                key="$this.comment_range.show",
                expression="return $this.comment_type.value == '{}'".format(CreateCommentType.CONTENT.value))
            ]),
            atomicMg.param("comment_all", dynamics=[DynamicsItem(
                key="$this.comment_all.show",
                expression="return $this.comment_type.value == '{}'".format(CreateCommentType.CONTENT.value))
            ]),
        ],
        outputList=[])
    def create_excel_comment(excel: ExcelObj, comment_type: CreateCommentType = CreateCommentType.POSITION,
                             comment: str = "", sheet_name: str = "", cell_position: str = "",
                             comment_range: SearchSheetType = SearchSheetType.ONE, find_str: str = "",
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            ...
            atomicMg.param("range_location", dynamics=[DynamicsItem(
                key="$this.range_location.show",
                expression="return $this.select_type.value == '{}'".format(ReadRangeType.AREA.value))
            ]),
        ],
        outputList=[])
    def excel_text_to_number(excel_obj: ExcelObj,
                             sheet_name: str = "",
                             select_type: ReadRangeType = ReadRangeType.CELL,
                             cell_position: str = '',
                             row: str = '',
    ...
    @atomicMg.atomic(
        ...
        outputList=[])
    def excel_number_to_text(excel_obj: ExcelObj,
                             sheet_name: str = "",
                             select_type: ReadRangeType = ReadRangeType.CELL,
                             cell_position: str = '',
                             row: str = '',
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            atomicMg.param("sheet_name", required=False),
            atomicMg.param("width", dynamics=[DynamicsItem(
                key="$this.width.show",
                expression="return $this.set_type.value == '{}'".format(SetType.VALUE.value))
            ]),
        ],
        outputList=[])
    def excel_set_col_width(excel_obj: ExcelObj, sheet_name: str, set_type: SetType = SetType.AUTO, col: str = "", width: str = ""):
        """
        设置指定列宽。
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            atomicMg.param("sheet_name", required=False),
            atomicMg.param("height", dynamics=[DynamicsItem(
                key="$this.height.show",
                expression="return $this.set_type.value == '{}'".format(SetType.VALUE.value))
            ]),
        ],
        outputList=[])
    def excel_set_row_height(excel_obj: ExcelObj, sheet_name: str, set_type: SetType = SetType.AUTO, row: str = "", height: str = ""):
        """
        设置指定行高。
        :param excel_obj: Excel对象
        :param sheet_name: 工作表名称
