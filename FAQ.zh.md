...
### Q: 🆕 多个客户端连接同一服务端，通过 API 运行工作流时如何指定在哪台客户端执行？

**A:** 通过 API 触发时，用对应客户端的 **API Key** 来指定要在哪一台客户端上执行（每个客户端各自有独立的 API Key）。需要注意：**远程计划任务（定时任务）是随机分配**到已连接的客户端上的，不保证固定在某一台执行。
...
### Q: 🆕 客户端启动时进度条卡在最后一步，日志显示调度器（scheduler）启动失败？

**A:** 客户端在解压 Python 核心包（`python_core.7z`）后会启动调度器服务（`astronverse.scheduler`）。若这一步失败（日志出现启动调度器命令返回错误码 1），通常是**本地缓存残留**或**账号权限不足**导致，可依次尝试：

1. **清理缓存后重启**：手动删除 `C:\Users\{用户名}\AppData\Roaming\astron-rpa\` 目录，重启电脑后再启动客户端。
2. **用管理员身份运行**：公司电脑的**域账号权限受限**时调度器可能无法启动，改用管理员身份运行客户端即可解决。
3. 若仍失败，可能是当前电脑环境不适配，请把操作系统版本提供给技术支持进一步排查。

### Q: 🆕 在 Casdoor 里新建了组织后，客户端点击登录报错？

**A:** 开源版 RPA 基于 Casdoor 做认证，但**只用到了 Casdoor 的部分能力，不支持多租户（多组织）**。服务端会固定映射到内置的那一个组织，因此请**只保留 1 个非 built-in 的组织**：

- 若在 Casdoor 后台额外新建了组织，登录时可能报错，删除多余组织后即可恢复。
- 内置的「示例组织」是内部功能、对终端用户不可见，可以修改其显示名称，但**不要新增额外组织**。
