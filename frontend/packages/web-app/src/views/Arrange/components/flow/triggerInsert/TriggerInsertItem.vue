...
const props = defineProps<{ id: string, index: number, indent: number }>()
...
watch(() => triggerIds.value, (val) => {
  preId.value = val[props.index - 1] || ''
  nextId.value = val[props.index + 1] || ''
}, { immediate: true })
...
function canInsert(pos: string) {
  ...
  if (triggerId.value === props.id && pos === inputPos.value) {
    return false
  }
  return true
}
...
function clickAtom(key: string) {
  const addToIndex = inputPos.value === 'top' ? props.index : props.index + 1
...
<template>
  ...
  >
    ...
    <div v-if="triggerId === props.id && inputPos === 'top'" class="trigger-insert-item-input">
      <TriggerInput @select="clickAtom" />
    </div>
    <slot />
    <div v-if="triggerId === props.id && inputPos === 'bottom'" class="trigger-insert-item-input">
      <TriggerInput @select="clickAtom" />
    </div>
  </div>
</template>
